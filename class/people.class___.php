...
class People extends Default_Table
{
	...
	function getProfile($profileId) {
    	...
    	$imageTitleData = '<div>'.$this->convert_high_bytes($this->convert_smart_quotes(stripslashes($profile[0]['info']))).'</div>####'.$profile[0]['name'].' '.stripslashes($profile[0]['lastName']).'##'.stripslashes($profile[0]['title']).'##';
