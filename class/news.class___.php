...
class News extends Default_Table
{
	...
	function getNews() {
		...
		//echo $readMore.'<br>';
		$imageTitleData = $this->convert_high_bytes($this->convert_smart_quotes(strip_tags($news['body'], '<div><p><br><object><param>')));
		...
		$imageTitleData = stripslashes($imageTitleData).'##'.$date.'<br>'.$news['sourceText'].'##'.stripslashes($news['title']).'## ##'.$readMore.'##'.$sideDate;
